...
async function getArtistInfo(access_token) {
  const response = await fetch("https://api.spotify.com/v1/artists/4oLeXFyACqeem2VImYeBFe", {
    method: 'GET',
...
getToken().then(response => {
  getArtistInfo(response.access_token).then(profile => {
    document.getElementById("artistinfo").textContent = profile.followers.total;
  })
